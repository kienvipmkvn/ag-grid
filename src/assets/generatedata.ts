export function generatedata(rowscount?: number, hasNullValues?: boolean): any[] {
    ...
    for (let i = 0; i < rowscount; i++) {
        ...
        row['firstname'] =i+" "+ firstNames[Math.floor(Math.random() * firstNames.length)];
        row['lastname'] =i+" "+  lastNames[Math.floor(Math.random() * lastNames.length)];
        row['name'] =i+" "+  row['firstname'] + ' ' + row['lastname']; 
        ...
        row['date'] = date;
        //row['orders'] = generateordersdata(4);
